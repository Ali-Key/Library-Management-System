// Create endpoints for borrownig, make sure to use the middleware to authenticateAdmin the token
...
router.post("/", authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "Borrowing successfully created!" , newBorrowing });
  } catch (error) {
    res.status(500).json({ status: 500, message: error.message });
  }
});
...
router.put("/:id", authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "Borrowing successfully updated!" , updatedBorrowing });
  } catch (error) {
    res.status(500).json({ status: 500, message: error.message });
  }
});
